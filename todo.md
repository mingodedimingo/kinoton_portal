...
- [x] 마이페이지 구현 (/mypage) - 로그인 유저 자동 매핑 + 프로필 이미지 업로드 + 연차/출퇴근 이력
- [x] 조직도 페이지 활성화 (/orgchart) - CSV 83명 DB 입력 + 본부별 계층 조직도 구현
- [x] 일정 페이지 활성화 (/calendar)
- [x] 전자결재 페이지 활성화 (/approve) - 이카운트 API 미제공으로 자체 UI 유지 (필터/통계 동작)
...
- [x] 메인 홈 게시판/공지/인사발령/경조사 항목 클릭 시 해당 상세 페이지로 이동 (팝업 아님)
- [x] 게시글/공지/인사발령/경조사 수정 버튼 및 수정 폼 구현 (어드민: 공지/인사/경조사, 작성자 본인: 게시판)
- [x] 게시글 상세에서 이미지 상단 배치, 본문 하단 배치로 레이아웃 변경

## UI 개선 5차 작업

- [x] 게시판 수정 폼에 이미지 업로드/교체 기능 추가
